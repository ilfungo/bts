...
//		return str_replace($search, $replace, $value);
	...
	public static function loadProducts($titleparam,$catparams,$attrparams,$priceparam,$saleparam,$customparam,&$total,$ispagination,$isnext,&$hasnext,&$isbegin,$categoryor,$skuparam,$tagsparams,$descparam,$shortdescparam)
	{
	try {
		global $wpdb;
...
//		$titlequery = "";
		...
		if(count($catparams) > 0 || count($attrparams) > 0 || count($tagsparams) > 0)
		{
			if(is_array($curr_settings))
			{
...
//			INNER JOIN {$term} rel ON {$posts}.ID=rel.object_id{$catsquery}
		if($bgettotalnumber)
		{
			$query = "INSERT INTO {$temptable} (
				...
				FROM {$posts}
				{$catsquery}{$pricequery}{$salequery}{$skuquery}
		...
		{//let's get products without product_type'
			if($bgettotalnumber)
			{
				$query = "INSERT INTO {$temptable} (
					SELECT 
					{$posts}.ID, 0 AS type, 0 AS post_parent,0 as useit 
					FROM {$posts}{$pricequery}{$salequery}{$skuquery}
							...
							if($i !== 'ID' && $i !== 'post_parent' && $i !== 'parent')
							{
								...
								}elseif(strpos($i,"attribute_pa_",0) === 0 && strpos($i,"_visiblefp",0) !== FALSE)
								{
									...
									if($ret === 'product')//check by post_type if($parentid == 0)
									{
										...
										$taxonomy_slug = str_replace('attribute_','',$i);
...
//				$attributename = $product_data['post_title'];
	...
	public static function duplicateProducts(&$data)
	{
		global $wpdb;
		$posts = $wpdb->posts;
		$meta = $wpdb->postmeta;
...
//			die ();
		...
		switch($type){
			case 'loadproducts':
			{
				...
				$ret = self::loadProducts($titleparam,$catparams,$attrparams,$priceparam,$saleparam,$customparam,$total,$ispagination,$isnext,$hasnext,$isbegin,$categoryor,$skuparam,$tagsparams,$descparam,$shortdescparam);
			...
			case 'duplicateproducts':
			{
				...
				$newcarr = array();
				self::convertSaveArrays($data,$newarr,$children,$newcarr);
				$ret = self::duplicateProducts($newarr);
				if(is_wp_error($ret) || -1 === $ret)
				{
					$arr['success'] = 'no';
					if(is_wp_error($ret))
