...
//	"use strict";
	...
	$('#butprevious').attr("disabled","disabled");
	...
	{
		{
			...
			<table cellpadding="4" cellspacing="0"> \
				<tr> \
					<td> \
						<label><input type="radio" value="0" checked name="deletewhat">'+movet+'</label> \
					</td></tr><tr><td>\
						<label><input type="radio" value="1" name="deletewhat">'+deletep+'</label> \
					</td> \
				</tr>  \
			</table>  \
	...
	$('body').on('mouseenter','#addprodarea',function()
	{
		...
		$('.addprodextra').show(100,function ()
		{
			...
			{
				$('#addprodwhatv').attr('disabled','disabled');
				return;
			}
			var hasone = false;
			var parentid = 0;
	 			 ...
	 			 if (_selectedParent.hasOwnProperty(key)) 
				 {
					 ...
					 if(true === col.attribute)
					 {
					 	if(_selectedParent[col.field + '_ids'] !== undefined && _selectedParent[col.field + '_ids'] != "")
						{
							...
							{
								$('#bulkvarsdialog input[value="'+attrarray[i]+'"]').removeAttr('disabled');
							}
						}
					 }
				 }
...
//					newitem.editor = Slick.Editors.Image;
	...
	$('body').on('change','#bulkdialog .selectvisiblefp',function(){
		...
		$parent.find(".selectvisiblefp").removeAttr("disabled");
		$parent.find(".selectusedforvars").removeAttr("disabled");
		var setvisval = $parent.find(".selectvisiblefp").val();
		if(setvisval == "skip")
		{
			$parent.find(".visiblefp").attr("disabled","disabled");
			$('#bulk' + column).attr('disabled', false).trigger("chosen:updated");
			$('#bulkadd' + column).removeAttr("disabled");
		}else if(setvisval == "andset")
		{
			$parent.find(".visiblefp").removeAttr("disabled");
			$('#bulk' + column).attr('disabled', false).trigger("chosen:updated");
			$('#bulkadd' + column).removeAttr("disabled");
		}else if(setvisval == "onlyset")
		{
			$parent.find(".visiblefp").removeAttr("disabled");
			$parent.find(".selectusedforvars").attr("disabled","disabled");
			$parent.find(".usedforvars").attr("disabled","disabled");
			$('#bulkadd' + column).attr("disabled","disabled");
			$('#bulk' + column).attr('disabled', true).trigger("chosen:updated");
		}
		if( $parent.find(".selectusedforvars").is(':enabled'))
		{
			...
			{
				$parent.find(".usedforvars").removeAttr("disabled");
			}else if(setvisval == "onlyset")
			{
				$parent.find(".usedforvars").removeAttr("disabled");
				$parent.find(".selectvisiblefp").attr("disabled","disabled");
				$parent.find(".visiblefp").attr("disabled","disabled");
				$('#bulkadd' + column).attr("disabled","disabled");
				$('#bulk' + column).attr('disabled', true).trigger("chosen:updated");
			}
	...
	$('body').on('click','#bulkdialog .bulkset',function(){
		...
		if(!item.prop('checked'))
		{
			if(coldef !== undefined && coldef.type === "customtaxh")
			{
				...
				{
					item.parent().parent().find(".selectvisiblefp").attr("disabled","disabled");
					item.parent().parent().find(".selectusedforvars").attr("disabled","disabled");
					item.parent().parent().find(".visiblefp").attr("disabled","disabled");
					item.parent().parent().find(".usedforvars").attr("disabled","disabled");
				}
			}
			else
				$('#bulk' + column).attr("disabled","disabled");
		}else
		{
			if(coldef !== undefined && coldef.type === "customtaxh")
			{
				...
				$('#bulk' + column).removeAttr("disabled");
		}
	});
...
//selection manager
	$('body').on('click','#selectdialog .selectset',function(){
		...
		if(!item.prop('checked'))
		{
			$('#selectdialog #select' + item.attr('data-id')).attr("disabled","disabled");
		}else
		{
			$('#selectdialog #select' + item.attr('data-id')).removeAttr("disabled");
		}
	});
...
//				 	BulkUpdateField(field,selitem,value,rowid,action)
	...
	$("#bulk_sale_price").change(function() 
    {
		...
		if(what == "delete")
		{
			$('#bulksalepricevalue').attr("disabled","disabled");
		}else
		{
			$('#bulksalepricevalue').removeAttr("disabled");
		}
	...
	function DisableAllControls(bdis)
    {
  		if(bdis)
	  	{
	  		$('#getproducts').attr("disabled","disabled");
			$('#savechanges').attr("disabled","disabled");
			$('#selectedit').attr("disabled","disabled");
			$('#bulkedit').attr("disabled","disabled");
			$('#butprevious').attr("disabled","disabled");
			$('#gotopage').attr("disabled","disabled");
			$('#butnext').attr("disabled","disabled");
			$('#revertcell').attr("disabled","disabled");
			$('#revertrow').attr("disabled","disabled");
			$('#revertall').attr("disabled","disabled");
			$('#deletebut').attr("disabled","disabled");
			$('#addprodbut').attr("disabled","disabled");
			$('#duplicateprodbut').attr("disabled","disabled");
		}else
		{
			$('#getproducts').removeAttr("disabled");
			$('#savechanges').removeAttr("disabled");
			$('#selectedit').removeAttr("disabled");
			$('#bulkedit').removeAttr("disabled");
			$('#deletebut').removeAttr("disabled");
			$('#addprodbut').removeAttr("disabled");
			$('#duplicateprodbut').removeAttr("disabled");
			if(_totalrecords > _recordslimit)
			{
				$('#butprevious').removeAttr("disabled");
				$('#gotopage').removeAttr("disabled");
				$('#butnext').removeAttr("disabled");
			}
			$('#revertcell').removeAttr("disabled");
			$('#revertrow').removeAttr("disabled");
			$('#revertall').removeAttr("disabled");
		}
	}
				     ...
				     success: function(response) {
							...
							for (var key in _arrData) 
							{
							  if (_arrData.hasOwnProperty(key)) 
							  {
								...
								if(obj.type !== undefined && obj.type=== 'customh')
								{
									if(response[obj.name] !== undefined)
									{
										$("#bulkdialog tr[data-id='" + obj.name + "']").html(response[obj.name]);
									}
