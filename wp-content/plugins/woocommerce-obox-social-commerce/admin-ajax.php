...
add_filter( 'stylesheet', array( &$ocmx_oboxfb_class, 'oboxfb_stylesheet'));
add_filter( 'theme_root', array( &$ocmx_oboxfb_class, 'oboxfb_template_dir'));
add_filter( 'theme_root_uri', array( &$ocmx_oboxfb_class, 'oboxfb_template_uri'));
add_filter( 'template', array( &$ocmx_oboxfb_class, 'oboxfb_template'));
...
if ( ! is_user_logged_in() ) {
	...
	if ( isset( $_POST['action'] ) && $_POST['action'] == 'autosave' ) {
		...
		$message = sprintf( __('<strong>ALERT: You are logged out!</strong> Could not save draft. <a href="%s" target="_blank">Please log in again.</a>', 'ocmx'), wp_login_url() );
...
function _wp_ajax_delete_comment_response( $comment_id ) {
	...
	$page_links = paginate_links( array(
		...
		'format' => '',
		'prev_text' => __('&laquo;','ocmx'),
		'next_text' => __('&raquo;','ocmx'),
...
case 'delete-link-cat' :
	...
	if ( $id == $default ) {
		$x = new WP_AJAX_Response( array(
			'what' => 'link-cat',
			'id' => $id,
			'data' => new WP_Error( 'default-link-cat', sprintf(__("Can&#8217;t delete the <strong>%s</strong> category: this is the default one","ocmx"), $cat_name) )
		) );
		$x->send();
	}
...
case 'dim-comment' : // On success, die with time() instead of 1
	...
	if ( !$comment = get_comment( $id ) ) {
		$x = new WP_Ajax_Response( array(
			'what' => 'comment',
			'id' => new WP_Error('invalid_comment', sprintf(__('Comment %d does not exist','ocmx'), $id))
		) );
		$x->send();
	}
...
case 'add-link-cat' : // From Blogroll -> Categories
	...
	if ( '' === trim($_POST['name']) ) {
		$x = new WP_Ajax_Response( array(
			'what' => 'link-cat',
			'id' => new WP_Error( 'name', __('You did not enter a category name.','ocmx') )
		) );
		$x->send();
	}
...
case 'add-tag' : // From Manage->Tags
	...
	if ( !$tag || is_wp_error($tag) || (!$tag = get_term( $tag['term_id'], $taxonomy )) ) {
		$message = __('An error has occured. Please reload the page and try again.','ocmx');
...
case 'get-tagcloud' :
	...
	if ( empty( $tags ) ) {
		$tax = get_taxonomy( $taxonomy );
		die( isset( $tax->no_tagcloud ) ? $tax->no_tagcloud : __('No tags found!','ocmx') );
	}
...
case 'replyto-comment' :
	...
	elseif ( in_array($status, array('draft', 'pending', 'trash') ) )
		die( __('Error: you are replying to a comment on a draft post.','ocmx') );
	...
	if ( $user->ID ) {
		...
		die( __('Sorry, you must be logged in to reply to a comment.','ocmx') );
	}
	...
	if ( isset($_POST['metakeyselect']) || isset($_POST['metakeyinput']) ) {
		...
		if ( $post->post_status == 'auto-draft' ) {
			...
			if ( $pid = edit_post() ) {
				...
				if ( !$mid = add_meta( $pid ) )
					die(__('Please provide a custom field value.','ocmx'));
			} else {
				die('0 15');
			}
		...
		if ( '' == trim($key) )
			die(__('Please provide a custom field name.','ocmx'));
		if ( '' == trim($value) )
			die(__('Please provide a custom field value.','ocmx'));
		if ( !$meta = get_post_meta_by_id( $mid ) )
			die('0 16'); // if meta doesn't exist
		if ( !current_user_can( 'edit_post', $meta->post_id ) )
			die('-1');
		if ( $meta->meta_value != stripslashes($value) || $meta->meta_key != stripslashes($key) ) {
			if ( !$u = update_meta( $mid, $key, $value ) )
				die('0 17'); // We know meta exists; we also know it's unchanged (or DB error, in which case there are bigger problems).
		}
...
case 'add-user' :
	...
	$x = new WP_Ajax_Response( array(
		...
		'supplemental' => array(
			'show-link' => sprintf(__( 'User <a href="#%s">%s</a> added','ocmx' ), "user-$user_id", $user_object->user_login),
...
case 'autosave' : // The name of this action is hardcoded in edit_post()
	...
	$draft_saved_date_format = __('g:i:s a','ocmx');
	/* translators: %s: date and time */
	$message = sprintf( __('Draft saved at %s.','ocmx'), date_i18n( $draft_saved_date_format ) );
	...
	if ( $last = wp_check_post_lock( $post->ID ) ) {
		...
		$last_user_name = $last_user ? $last_user->display_name : __( 'Someone','ocmx' );
		$data = new WP_Error( 'locked', sprintf(
			$_POST['post_type'] == 'page' ? __( 'Autosave disabled: %s is currently editing this page.','ocmx' ) : __( 'Autosave disabled: %s is currently editing this post.','ocmx' ),
	...
	if ( 'page' == $post->post_type ) {
		if ( !current_user_can('edit_page', $post_ID) )
			die(__('You are not allowed to edit this page.','ocmx'));
	} else {
		if ( !current_user_can('edit_post', $post_ID) )
			die(__('You are not allowed to edit this post.','ocmx'));
	}
...
case 'inline-save':
	...
	if ( $last = wp_check_post_lock( $post_ID ) ) {
		$last_user = get_userdata( $last );
		$last_user_name = $last_user ? $last_user->display_name : __( 'Someone','ocmx' );
		printf( $_POST['post_type'] == 'page' ? __( 'Saving is disabled: %s is currently editing this page.','ocmx' ) : __( 'Saving is disabled: %s is currently editing this post.','ocmx' ),	esc_html( $last_user_name ) );
		exit;
	}
	...
	switch ($_POST['tax_type']) {
		case 'link-cat' :
			...
			if ( $updated && !is_wp_error($updated) )
				...
				die( __('Category not updated.','ocmx') );
...
case 'find_posts':
	...
	$html = '<table class="widefat" cellspacing="0"><thead><tr><th class="found-radio"><br /></th><th>'.__('Title','ocmx').'</th><th>'.__('Date','ocmx').'</th><th>'.__('Status','ocmx').'</th></tr></thead><tbody>';
	foreach ( $posts as $post ) {

		switch ( $post->post_status ) {
			case 'publish' :
			case 'private' :
				$stat = __('Published','ocmx');
				break;
			case 'future' :
				$stat = __('Scheduled','ocmx');
				break;
			case 'pending' :
				$stat = __('Pending Review','ocmx');
				break;
			case 'draft' :
				$stat = __('Draft','ocmx');
				break;
		...
		if ( '0000-00-00 00:00:00' == $post->post_date ) {
			...
			$time = mysql2date(__('Y/m/d','ocmx'), $post->post_date);
		}
...
case 'save-widget' :
	...
	$error = '<p>' . __('An error has occured. Please reload the page and try again.','ocmx') . '</p>';
