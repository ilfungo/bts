<?php function oboxfb_breadcrumbs($container_id = 'crumbs') {
	global $wp_query, $post;

	if(get_option("oboxfb_breadcrumbs") != 'yes')
		return; ?>

	<ul id="<?php echo $container_id; ?>" class="clearfix">
		...
		*/
		...
		<?php } elseif( function_exists('is_shop') && ( is_post_type_archive( 'product' ) || ( get_post_type() == "product") ) ) { ?>
			<li> / </li>
			<?php if( function_exists( 'woocommerce_get_page_id' )  && '' != woocommerce_get_page_id('shop') && '-1' != woocommerce_get_page_id('shop') ) {
				$shop_page = get_post( woocommerce_get_page_id('shop') ); ?>
				<li><a href="<?php echo get_permalink( $shop_page->ID ); ?>"><?php echo $shop_page->post_title; ?></a></li>
			<?php } else { ?>
				<li><a href=""><?php _e( 'All Products' , 'ocmx' ); ?></li>
			<?php }
